...
class ModelRecyclerAdapter<M : BaseModel, VH : BaseViewHolder<*, M>>(
    private var modelList: List<BaseModel>,
    private val adapterListener: AdapterListener
) : ListAdapter<BaseModel, VH>(BaseModel.ModelDiffCallback) {

    override fun getItemViewType(position: Int): Int = modelList[position].cellType.ordinal

    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): VH = ViewHolderMapper.map(
